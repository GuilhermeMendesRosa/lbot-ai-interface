...
frontLeftWheel.position.set(-11, 4, 10);
frontLeftWheel.castShadow = true;
robotGroup.add(frontLeftWheel);

const frontRightWheel = new THREE.Mesh(wheelGeometry, wheelMaterial);
frontRightWheel.rotation.z = Math.PI / 2;
frontRightWheel.position.set(11, 4, 10);
frontRightWheel.castShadow = true;
robotGroup.add(frontRightWheel);

const rearLeftWheel = new THREE.Mesh(wheelGeometry, wheelMaterial);
rearLeftWheel.rotation.z = Math.PI / 2;
rearLeftWheel.position.set(-11, 4, -10);
rearLeftWheel.castShadow = true;
robotGroup.add(rearLeftWheel);

const rearRightWheel = new THREE.Mesh(wheelGeometry, wheelMaterial);
rearRightWheel.rotation.z = Math.PI / 2;
rearRightWheel.position.set(11, 4, -10);
rearRightWheel.castShadow = true;
robotGroup.add(rearRightWheel);
...
const frontLeftWheelDetail = new THREE.Mesh(
  wheelDetailGeometry,
  wheelDetailMaterial
);
frontLeftWheelDetail.rotation.z = Math.PI / 2;
frontLeftWheelDetail.position.set(-11, 4, 10);
robotGroup.add(frontLeftWheelDetail);

const frontRightWheelDetail = new THREE.Mesh(
  wheelDetailGeometry,
  wheelDetailMaterial
);
frontRightWheelDetail.rotation.z = Math.PI / 2;
frontRightWheelDetail.position.set(11, 4, 10);
robotGroup.add(frontRightWheelDetail);

const rearLeftWheelDetail = new THREE.Mesh(
  wheelDetailGeometry,
  wheelDetailMaterial
);
rearLeftWheelDetail.rotation.z = Math.PI / 2;
rearLeftWheelDetail.position.set(-11, 4, -10);
robotGroup.add(rearLeftWheelDetail);

const rearRightWheelDetail = new THREE.Mesh(
  wheelDetailGeometry,
  wheelDetailMaterial
);
rearRightWheelDetail.rotation.z = Math.PI / 2;
rearRightWheelDetail.position.set(11, 4, -10);
robotGroup.add(rearRightWheelDetail);
...
// =====================================================================
// ESTADO DO ROBÔ E CONFIGURAÇÕES
// =====================================================================
...
const ROBOT_SPEED = 30; // unidades por segundo
const ROTATION_SPEED = 90; // graus por segundo

// Limites da arena (considerando o tamanho do robô)
const ARENA_LIMIT = 190; // 200 - 10 (margem de segurança)

// =====================================================================
// FUNÇÕES DE VALIDAÇÃO E MOVIMENTO
// =====================================================================

/**
 * Verifica se uma posição está dentro dos limites da arena
 */
...
/**
 * Calcula a posição máxima possível sem ultrapassar barreiras
 */
...
/**
 * Executa um comando LBML individual
 */
async function executeCommand(cmd) {
  ...
  document.getElementById(
    'currentCommand'
  ).textContent = `${cmd.type}${cmd.value}${cmd.direction}`;
  ...
  if (cmd.type === 'D') {
    ...
    switch (cmd.direction) {
      case 'F': {
        ...
        const resultF = getMaxValidPosition(
          robotState.x,
          robotState.z,
          targetXF,
          targetZF
        );
        const actualDistanceF = Math.sqrt(
          Math.pow(resultF.x - robotState.x, 2) +
            Math.pow(resultF.z - robotState.z, 2)
        );
      ...
      case 'B': {
        ...
        const resultB = getMaxValidPosition(
          robotState.x,
          robotState.z,
          targetXB,
          targetZB
        );
        const actualDistanceB = Math.sqrt(
          Math.pow(resultB.x - robotState.x, 2) +
            Math.pow(resultB.z - robotState.z, 2)
        );
      ...
      case 'L': {
        ...
        const resultL = getMaxValidPosition(
          robotState.x,
          robotState.z,
          targetXL,
          targetZL
        );
        const actualDistanceL = Math.sqrt(
          Math.pow(resultL.x - robotState.x, 2) +
            Math.pow(resultL.z - robotState.z, 2)
        );
      ...
      case 'R': {
        ...
        const resultR = getMaxValidPosition(
          robotState.x,
          robotState.z,
          targetXR,
          targetZR
        );
        const actualDistanceR = Math.sqrt(
          Math.pow(resultR.x - robotState.x, 2) +
            Math.pow(resultR.z - robotState.z, 2)
        );
    ...
    const targetRotation =
      robotState.rotation + (cmd.direction === 'R' ? -angle : angle);
    await animateRotation(targetRotation, angle);
  }
}

/**
 * Anima o movimento linear do robô
 */
...
function animateRotation(targetRotation, angle) {
  return new Promise((resolve) => {
    ...
    function animate() {
      ...
      // Efeito de inclinação durante a rotação
...
/**
 * Executa uma sequência de comandos LBML
 */
...
// =====================================================================
// CONTROLE DA CÂMERA E RENDERIZAÇÃO
// =====================================================================
...
// =====================================================================
// INTERFACE DE MENSAGENS (POSTMESSAGE)
// =====================================================================

const ORIGIN_ANY = '*';

/**
 * Envia uma mensagem de confirmação para o parent
 */
function postAck(type, payload) {
  try {
    parent.postMessage({ type, ...payload }, ORIGIN_ANY);
  } catch (err) {
...
/**
 * Listener para mensagens recebidas do parent
 */
